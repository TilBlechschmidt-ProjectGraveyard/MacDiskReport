...
class FilesystemScan {
    ...
    func scan() {
        ...
        let initialDirectory = Directory(name: baseURL.lastPathComponent, size: 0, location: baseURL, parent: nil)
        ...
        for case let fileURL as URL in enumerator {
            ...
            let name = node.name ?? fileURL.lastPathComponent
            ...
            if isDirectory {
                directoryCount += 1
                let newDirectory = Directory(name: name, size: 0, location: fileURL, parent: workingDirectory.last)
                workingDirectory.append(newDirectory)
            } else if let parent = workingDirectory.last {
                fileCount += 1
                let newFile = File(name: name, size: node.fileAllocatedSize ?? 0, location: fileURL, parent: parent)
